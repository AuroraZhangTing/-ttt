...
app.get('/api/projects', auth, async (req, res) => {
  ...
  const result = await Promise.all(projects.map(async (p) => {
    const { data: steps } = await supabase
      .from('project_steps').select('step_index, status')
      .eq('project_id', p.id).order('step_index');
...
app.post('/api/projects', auth, async (req, res) => {
  ...
  const { data: project, error } = await supabase
    .from('projects').insert({ name, total_amount: total_amount || 0 }).select().single();
  if (error) return res.status(500).json({ error: error.message });
  const stepsData = Array.from({ length: 7 }, (_, i) => ({
    project_id: project.id, step_index: i, status: 'pending', data: '{}'
  }));
...
app.get('/api/projects/:id/steps', auth, async (req, res) => {
  const { data: project } = await supabase
    .from('projects').select('*').eq('id', parseInt(req.params.id)).single();
  if (!project) return res.status(404).json({ error: '项目不存在' });
  const { data: steps } = await supabase
    .from('project_steps').select('*').eq('project_id', parseInt(req.params.id)).order('step_index');
...
app.put('/api/projects/:id/steps/:stepIndex', auth, async (req, res) => {
  ...
  const { data: existing } = await supabase
    .from('project_steps').select('data')
    .eq('project_id', parseInt(req.params.id)).eq('step_index', parseInt(req.params.stepIndex)).single();
  ...
  const updateObj = { data: JSON.stringify(existingData), updated_at: new Date().toISOString() };
  if (status) updateObj.status = status;
  await supabase.from('project_steps').update(updateObj)
    .eq('project_id', parseInt(req.params.id)).eq('step_index', parseInt(req.params.stepIndex));
  await supabase.from('projects').update({ updated_at: new Date().toISOString() })
    .eq('id', parseInt(req.params.id));
  const { data: updated } = await supabase.from('project_steps').select('*')
    .eq('project_id', parseInt(req.params.id)).eq('step_index', parseInt(req.params.stepIndex)).single();
...
app.get('/api/projects/:id/steps/:stepIndex', auth, async (req, res) => {
  const { data: step } = await supabase.from('project_steps').select('*')
    .eq('project_id', parseInt(req.params.id)).eq('step_index', parseInt(req.params.stepIndex)).single();
